...
namespace WindowsFormsApp1
{
	public class frmAttendance : Form
	{
		...
		private void BuildUI()
		{
			...
			toolbar.Controls.Add(new Label { Text = "Date:", AutoSize = true, Location = new Point(188, 17), ForeColor = TxtSec });
			dtpDate = new DateTimePicker { Size = new Size(200, 28), Location = new Point(225, 12), Font = new Font("Segoe UI", 9f), Value = DateTime.Today };
			dtpDate.ValueChanged += (s, e) => LoadData();

			toolbar.Controls.Add(new Label { Text = "Shift:", AutoSize = true, Location = new Point(440, 17), ForeColor = TxtSec });
			...
			dgv.Columns.Add("attendance_id", "Att. ID");
			...
			dgv.Columns.Add("work_hours", "Work Hrs");
			dgv.Columns.Add("overtime_hours", "Overtime Hrs");
			...
			string sql = @"
                SELECT 
                    ...
                    CASE
                        ...
                        WHEN CAST(a.check_in_time AS TIME) > '09:00' THEN 'Late'
                        ELSE 'Present'
                    END AS status
                FROM Attendance a
                ...
                WHERE 
                    ...
                    AND (? = '' OR e.full_name LIKE '%' + ? + '%' OR CAST(a.employee_id AS VARCHAR) LIKE '%' + ? + '%')
	...
	public class frmAttendanceAdd : Form
	{
		...
		private void BuildUI()
		{
			var pnl = new Panel { Dock = DockStyle.Fill, Padding = new Padding(20, 16, 20, 10), AutoScroll = true, BackColor = BgPage };
			int y = 0;
		...
		private void BtnSave_Click(object sender, EventArgs e)
		{
			...
			try
			{
				using (var con = new OdbcConnection(Global.ConnStr))
				{
					...
					// Check for duplicate
					using (var chk2 = new OdbcCommand(@"SELECT COUNT(1) FROM Attendance WHERE employee_id = ? AND CONVERT(DATE, date) = ?", con))
					{
						chk2.Parameters.AddWithValue("?", empId);
						chk2.Parameters.AddWithValue("?", dtpDate.Value.Date);
						if (Convert.ToInt32(chk2.ExecuteScalar()) > 0)
						{
							MessageBox.Show("Attendance record already exists for this employee on this date.", "Duplicate", MessageBoxButtons.OK, MessageBoxIcon.Warning);
							return;
						}
					}

					string insertSql = @"
                        INSERT INTO Attendance (employee_id, date, check_in_time, check_out_time, shift_type, work_hours, overtime_hours)
                        VALUES (?, ?, ?, ?, ?, ?, ?)";
					...
					MessageBox.Show($"Attendance saved successfully!\nWork Hours: {workHrs:F2}h | Overtime: {otHrs:F2}h", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
				}
