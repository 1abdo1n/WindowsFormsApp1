...
namespace WindowsFormsApp1
{
	public class frmMainNew : Form
	{
		...
		private void BuildUI()
		{
			...
			var lblBrand = MakeLabel(Global.AppName, new Font("Segoe UI", 13f, FontStyle.Bold), Color.White, new Point(18, 22));
			...
			// Welcome text
			var lblWelcome = MakeLabel($"Welcome back, {Global.LoggedUserFullName}!", new Font("Segoe UI", 15f, FontStyle.Bold), TxtPrimary, new Point(0, 0));
		...
		private void LoadDashboardData()
		{
			...
			try
			{
				...
				{
					// Manager stats - department specific
					...
					using (var con = new OdbcConnection(Global.ConnStr))
					{
                            ...
                            WHERE d.department_name = ?", con))
						{
							...
							teamCount = Convert.ToInt32(cmd.ExecuteScalar());
						}
						...
						{
							cmd.Parameters.AddWithValue("?", Global.LoggedUserDepartment);
							assetsCount = Convert.ToInt32(cmd.ExecuteScalar());
						}
				...
				else
				{
					...
					using (var con = new OdbcConnection(Global.ConnStr))
					{
						...
						// Get remaining vacation days (assuming 22 days total)
						using (var cmd = new OdbcCommand(@"
                            ...
                            WHERE employee_id = ? AND approval_status = 'Approved' AND YEAR(start_date) = YEAR(GETDATE())", con))
						{
							...
							int usedDays = Convert.ToInt32(cmd.ExecuteScalar());
							leaveDays = Math.Max(0, 22 - usedDays);
						}

						using (var cmd = new OdbcCommand(@"
                            SELECT COUNT(*) FROM [Leave] WHERE employee_id = ? AND approval_status = 'Pending'", con))
						{
							cmd.Parameters.AddWithValue("?", empId);
							pendingLeaves = Convert.ToInt32(cmd.ExecuteScalar());
						}
						...
						using (var cmd = new OdbcCommand(@"
                            SELECT ISNULL(AVG(final_rating), 0) FROM Performance_Evaluation WHERE employee_id = ? AND evaluation_year = YEAR(GETDATE())", con))
						{
							cmd.Parameters.AddWithValue("?", empId);
							avgPerformance = Convert.ToDouble(cmd.ExecuteScalar());
						}

						using (var cmd = new OdbcCommand(@"
                            SELECT TOP 1 net_salary FROM Payroll WHERE employee_id = ? ORDER BY pay_date DESC", con))
						{
							cmd.Parameters.AddWithValue("?", empId);
			...
			{
				// Fallback to static stats if database fails
				if (Global.IsAdmin)
				{
		...
		private void LoadDepartmentsOverview()
		{
			...
			try
			{
                    ...
                    ORDER BY d.department_name", con))
				{
					...
					while (reader.Read())
					{
						...
						row.Width = deptCard.Width;
						row.Controls.Add(MakeLabel(reader["department_name"].ToString(), new Font("Segoe UI", 9.5f), TxtPrimary, new Point(16, 10)));
						row.Controls.Add(MakeLabel(reader["emp_count"].ToString(), new Font("Segoe UI", 9.5f, FontStyle.Bold), Blue, new Point(260, 10)));
			...
			{
				deptCard.Controls.Add(MakeLabel("Error loading departments: " + ex.Message, new Font("Segoe UI", 9.5f), Color.Red, new Point(16, 48)));
				deptCard.Height = 100;
			}
		}
		...
		private void LoadTeamOverview()
		{
			...
			try
			{
                    ...
                    ORDER BY e.full_name", con))
				{
					...
					while (reader.Read())
					{
						...
						row.Width = deptCard.Width;
						row.Controls.Add(MakeLabel(reader["full_name"].ToString(), new Font("Segoe UI", 9.5f), TxtPrimary, new Point(16, 10)));
						row.Controls.Add(MakeLabel(reader["job_title"]?.ToString() ?? "Staff", new Font("Segoe UI", 9.5f), TxtGray, new Point(260, 10)));
						deptCard.Controls.Add(row);
						content.Resize += (s, e) => row.Width = deptCard.Width;
						i++;
					}
			...
			{
				deptCard.Controls.Add(MakeLabel("Error loading team: " + ex.Message, new Font("Segoe UI", 9.5f), Color.Red, new Point(16, 48)));
				deptCard.Height = 100;
			}
		}
		...
		private void LoadEmployeeInfo()
		{
			...
			try
			{
                    ...
                    WHERE e.employee_id = ?", con))
				{
					...
					if (reader.Read())
					{
						...
						deptCard.Controls.Add(MakeLabel(reader["full_name"].ToString(), new Font("Segoe UI", 9.5f), TxtPrimary, new Point(120, 20)));
						deptCard.Controls.Add(MakeLabel("Department:", new Font("Segoe UI", 9f, FontStyle.Bold), Color.FromArgb(55, 65, 81), new Point(16, 55)));
						deptCard.Controls.Add(MakeLabel(reader["department_name"]?.ToString() ?? "Not Assigned", new Font("Segoe UI", 9.5f), TxtPrimary, new Point(120, 55)));
						deptCard.Controls.Add(MakeLabel("Position:", new Font("Segoe UI", 9f, FontStyle.Bold), Color.FromArgb(55, 65, 81), new Point(16, 90)));
						deptCard.Controls.Add(MakeLabel(reader["job_title"]?.ToString() ?? "Staff", new Font("Segoe UI", 9.5f), TxtPrimary, new Point(120, 90)));
						deptCard.Controls.Add(MakeLabel("Hire Date:", new Font("Segoe UI", 9f, FontStyle.Bold), Color.FromArgb(55, 65, 81), new Point(16, 125)));
						deptCard.Controls.Add(MakeLabel(Convert.ToDateTime(reader["hire_date"]).ToString("dd/MM/yyyy"), new Font("Segoe UI", 9.5f), TxtPrimary, new Point(120, 125)));
						deptCard.Height = 170;
					}
					else
					{
						deptCard.Controls.Add(MakeLabel("No data found", new Font("Segoe UI", 9.5f), TxtGray, new Point(16, 48)));
						deptCard.Height = 80;
					}
				}
			}
			...
			{
				deptCard.Controls.Add(MakeLabel("Error loading data: " + ex.Message, new Font("Segoe UI", 9.5f), Color.Red, new Point(16, 48)));
				deptCard.Height = 100;
			}
		}
		...
		private void NavBtn_Click(object sender, EventArgs e)
		{
			...
			else if (Global.IsManager)
			{
				switch (name)
				{
					...
					case "Leave": frm = new frmLeave(Global.LoggedUserDepartment); break;
			...
			else
			{
				switch (name)
				{
					...
					case "My Leave": frm = new frmLeave("", Global.LoggedUserId.ToString()); break;
					case "My Contract":
