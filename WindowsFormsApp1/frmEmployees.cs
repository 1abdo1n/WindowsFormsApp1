...
namespace WindowsFormsApp1
{
	public class frmEmployees : Form
	{
		...
		private void BtnDelete_Click(object sender, EventArgs e)
		{
			...
			if (MessageBox.Show($"Are you sure you want to delete '{empName}'?\n\nThis will permanently delete all their records (attendance, payroll, leaves, assets, performance, contracts, etc.)!",
			...
			try
			{
				using (var con = new OdbcConnection(Global.ConnStr))
				{
					...
					// الجداول المرتبطة بـ FK بترتيب الحذف الصح
					// (الأطفال قبل الآباء)
					...
					{
						using (var cmd = new OdbcCommand($"DELETE FROM {table} WHERE employee_id = ?", con))
						{
					...
					// فك ربط الموظف بالعقد (NULL) بدل ما نحذف العقد نفسه
					using (var cmdContract = new OdbcCommand("UPDATE Employee SET contract_id = NULL WHERE employee_id = ?", con))
					{
						cmdContract.Parameters.AddWithValue("?", empId);
						cmdContract.ExecuteNonQuery();
					}

					// أخيراً حذف الموظف نفسه
					using (var cmdEmp = new OdbcCommand("DELETE FROM Employee WHERE employee_id = ?", con))
					{
						cmdEmp.Parameters.AddWithValue("?", empId);
						int rows = cmdEmp.ExecuteNonQuery();

						if (rows > 0)
						{
							MessageBox.Show("Employee and all related records deleted successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
							LoadData();
						}
						else
						{
	...
	public class frmEmployeeAddEdit : Form
	{
		...
		private void LoadDropdowns()
		{
			try
			{
				using (var con = new OdbcConnection(Global.ConnStr))
				{
					...
					using (var cmd = new OdbcCommand("SELECT contract_id FROM Contract WHERE status = 'Active' ORDER BY contract_id", con))
					{
						var reader = cmd.ExecuteReader();
						var dt = new System.Data.DataTable();
						dt.Load(reader);
			...
			try
			{
				...
				using (var cmd = new OdbcCommand(@"
                    SELECT full_name, national_id, insurance_number, medical_card_number,
                           blood_type, phone_number, email, department_id, job_title,
                           grade, contract_id, hire_date, status
		...
		private void BtnSave_Click(object sender, EventArgs e)
		{
			...
			// إضافة التحقق من رقم التليفون (أرقام فقط + طول 11 رقم)
			if (!string.IsNullOrWhiteSpace(txtPhone.Text))
			{
				if (!System.Text.RegularExpressions.Regex.IsMatch(txtPhone.Text, @"^\d+$"))
				{
					MessageBox.Show("رقم التليفون يجب أن يحتوي على أرقام فقط.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
					return;
				}

				if (txtPhone.Text.Length != 11)
				{
					MessageBox.Show("عفواً، يجب أن يتكون رقم التليفون من 11 رقم بالضبط.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning);
					return;
				}
			}
