...
namespace WindowsFormsApp1
{
	public class frmLeave : Form
	{
		...
		private readonly string userRole; // "Admin" | "Manager" | "Employee"
		...
		public frmLeave(string department = "", string employeeId = "", string role = "Employee")
		{
			...
			Text = string.IsNullOrEmpty(department)
				? "Leave Management"
				: $"Leave Management - {department} Department";
		...
		private void BuildUI()
		{
			...
			Label MakeStatCard(string lbl, Color clr, ref Label valLabel)
			{
				var card = new Panel
				{
				...
				card.Paint += (s2, e2) =>
					((Panel)s2).CreateGraphics()
							   .DrawRectangle(new Pen(Border), 0, 0, card.Width - 1, card.Height - 1);
				valLabel = new Label
				{
					Text = "0",
			...
			MakeStatCard("Pending", Pending, ref lblPending);
			MakeStatCard("Approved", Approved, ref lblApproved);
			MakeStatCard("Rejected", Rejected, ref lblRejected);
			if (userRole == "Admin" || userRole == "Manager")
				MakeStatCard("Total Requests", Blue, ref lblTotal);
			...
			toolbar.Paint += (s, e) =>
				e.Graphics.DrawLine(new Pen(Border), 0, 51, toolbar.Width, 51);
			...
			dgv.Columns.Add("leave_type", "Leave Type"); dgv.Columns["leave_type"].FillWeight = 75;
			dgv.Columns.Add("start_date", "Start Date"); dgv.Columns["start_date"].FillWeight = 80;
			dgv.Columns.Add("end_date", "End Date"); dgv.Columns["end_date"].FillWeight = 80;
			dgv.Columns.Add("days", "Days"); dgv.Columns["days"].FillWeight = 45;
			dgv.Columns.Add("approval_status", "Status"); dgv.Columns["approval_status"].FillWeight = 70;
			dgv.Columns.Add("approved_by_name", "Approved By"); dgv.Columns["approved_by_name"].FillWeight = 80;
		...
		private void LoadData()
		{
			...
			string sql = @"
                SELECT
                    ...
                    l.approval_status,
                    -- ✅ اسم الشخص الحقيقي اللي عمل approve/reject
                    ISNULL(apr.full_name, '—')           AS approved_by_name
                FROM [Leave] l
			...
			try
			{
				...
				using (var cmd = new OdbcCommand(sql, con))
				{
					...
					using (var rdr = cmd.ExecuteReader())
					{
						while (rdr.Read())
						{
							string status = rdr["approval_status"]?.ToString() ?? "Pending";
							switch (status)
							{
							...
							dgv.Rows.Add(
								rdr["leave_id"].ToString(),
								rdr["employee_id"].ToString(),
								rdr["full_name"]?.ToString() ?? "—",
								rdr["leave_type"]?.ToString() ?? "—",
								rdr["start_date_display"].ToString(),
								rdr["end_date_display"].ToString(),
								rdr["days"].ToString(),
								status,
								rdr["approved_by_name"]?.ToString() ?? "—"
							);
						}
					}
		...
		// ════════════════════════════════════════════════════════════════════
		//  ✅ Helper — يرجع employee_id للشخص المسجل دخوله دلوقتي
		// ════════════════════════════════════════════════════════════════════
		private int GetCurrentApproverId()
		{
			// Global.LoggedUserID هو الـ employee_id للشخص اللي عمل login
			// لو مش موجود بنرجع employee_id من userId اللي اتبعت للفورم
			if (Global.LoggedUserID > 0)
				return Global.LoggedUserID;

			if (!string.IsNullOrEmpty(userId) && int.TryParse(userId, out int id))
				return id;

			return 0; // fallback — مش المفروض يوصل هنا
		}
		...
		private void BtnApprove_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("Please select a leave request first.",
					"Approve", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}
			...
			{
				MessageBox.Show($"This request is already {currentStatus}.",
					"Cannot Approve", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}

			string empName = dgv.CurrentRow.Cells["full_name"].Value?.ToString() ?? "employee";
			if (MessageBox.Show($"Approve {empName}'s leave request?",
					"Confirm Approval", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
				!= DialogResult.Yes) return;

			try
			{
				int approverId = GetCurrentApproverId(); // ✅ اسم الشخص الحقيقي

				using (var con = new OdbcConnection(Global.ConnStr))
				using (var cmd = new OdbcCommand(
					"UPDATE [Leave] SET approval_status = 'Approved', approved_by = ? WHERE leave_id = ?", con))
				{
					cmd.Parameters.AddWithValue("?", approverId);
		...
		private void BtnReject_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("Please select a leave request first.",
					"Reject", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}
			...
			{
				MessageBox.Show($"This request is already {currentStatus}.",
					"Cannot Reject", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}

			string empName = dgv.CurrentRow.Cells["full_name"].Value?.ToString() ?? "employee";
			if (MessageBox.Show($"Reject {empName}'s leave request?",
			...
			try
			{
				...
				using (var con = new OdbcConnection(Global.ConnStr))
				using (var cmd = new OdbcCommand(
					"UPDATE [Leave] SET approval_status = 'Rejected', approved_by = ? WHERE leave_id = ?", con))
				{
					cmd.Parameters.AddWithValue("?", approverId);
					cmd.Parameters.AddWithValue("?", leaveId);
					con.Open();
		...
		private void Dgv_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
		{
			...
			if (row.Cells["approval_status"].Value == null || row.Selected) return;

			switch (row.Cells["approval_status"].Value.ToString())
			{
				case "Approved": row.DefaultCellStyle.BackColor = ApprovedBg; break;
				case "Pending": row.DefaultCellStyle.BackColor = PendingBg; break;
				case "Rejected": row.DefaultCellStyle.BackColor = RejectedBg; break;
	...
	public class frmLeaveAdd : Form
	{
		...
		private void CalculateDays()
		{
			...
			{
				lblDaysCalc.Text = "⚠ End date cannot be before start date!";
				lblDaysCalc.ForeColor = Color.FromArgb(185, 28, 28);
				return;
			}
			int days = (dtpEnd.Value.Date - dtpStart.Value.Date).Days + 1;
		...
		private void BtnSubmit_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show("Employee ID must be a number.", "Validation",
					MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
			}
			if (dtpEnd.Value.Date < dtpStart.Value.Date)
			{
				MessageBox.Show("End date cannot be before start date.", "Validation",
					MessageBoxButtons.OK, MessageBoxIcon.Warning); return;
			}
			...
			try
			{
				using (var con = new OdbcConnection(Global.ConnStr))
				{
					...
					using (var chk = new OdbcCommand(
						"SELECT COUNT(1) FROM Employee WHERE employee_id = ?", con))
					{
					...
					using (var chk2 = new OdbcCommand(@"
                        ...
                        WHERE employee_id = ?
                          ...
                          AND (   (start_date BETWEEN ? AND ?)
                               ...
                               OR (? BETWEEN start_date AND end_date))", con))
					{
						...
						{
							MessageBox.Show("You already have an overlapping leave request.",
					...
					using (var cmd = new OdbcCommand(@"
                        INSERT INTO [Leave] (employee_id, leave_type, start_date, end_date, approval_status)
                        VALUES (?, ?, ?, ?, 'Pending')", con))
					{
