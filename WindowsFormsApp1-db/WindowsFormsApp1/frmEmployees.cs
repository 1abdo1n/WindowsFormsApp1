...
namespace WindowsFormsApp1
{
    public class frmEmployees : Form
    {
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show($"Are you sure you want to delete '{empName}'?\n\nThis will also delete their user account and all related records (attendance, payroll, leaves, etc.)!",
            ...
            try
            {
                using (var con = new OdbcConnection(Global.ConnStr))
                {
                    ...
                    // 1. مسح حساب اليوزر المرتبط بالموظف الأول (لتجنب خطأ الـ Foreign Key)
                    using (var cmdUser = new OdbcCommand("DELETE FROM [User] WHERE employee_id = ?", con))
                    {
                        cmdUser.Parameters.AddWithValue("?", empId);
                        cmdUser.ExecuteNonQuery();
                    }

                    // 2. مسح الموظف نفسه بعد ما تم مسح حساب اليوزر المرتبط بيه
