...
namespace WindowsFormsApp1
{
	public class frmLogin : Form
	{
		...
		private TabPage BuildLoginTab(TabControl tabs)
		{
			...
			var lnkContact = new LinkLabel
			{
				Text = "Forgot password? Contact HR",
				Font = new Font("Segoe UI", 8.5f),
				LinkColor = Blue,
				AutoSize = true,
				Location = new Point(24, 302)
			};
			lnkContact.Click += (s, e) =>
				MessageBox.Show(
					"Contact your HR administrator to reset your password.\n\nEmail: hr@hrsystem.com",
					"Contact HR", MessageBoxButtons.OK, MessageBoxIcon.Information);
			...
			btnLogin.Click += (s, e) =>
			{
				...
				if (user != null)
				{
					...
					// ✅ تسجيل الحضور تلقائياً عند الـ Login (Admin, Manager, Employee)
					RecordAttendanceOnLogin(user.EmployeeId, user.Role);
		...
		// ────────────────────────────────────────────────────────────────────
		private void RecordAttendanceOnLogin(int employeeId, string role)
		{
			try
			{
				using (var con = new OdbcConnection(Global.ConnStr))
				{
					...
					// ── تحقق إن مفيش ريكورد للنهارده خالص ──────────────────
					using (var chk = new OdbcCommand(@"
                        SELECT COUNT(1) FROM Attendance
                        WHERE employee_id = ?
                          AND CONVERT(DATE, date) = CONVERT(DATE, GETDATE())", con))
					{
						chk.Parameters.AddWithValue("?", employeeId);
						if (Convert.ToInt32(chk.ExecuteScalar()) > 0)
							return; // الحضور متسجل قبل كده النهارده — مش هنعمل إدخال تاني
					}

					// ── تحديد الشيفت تلقائياً بناءً على وقت الدخول ──────────
					// (نفس المنطق للكل: Employee, Manager, Admin)
					TimeSpan now = DateTime.Now.TimeOfDay;
					string shift = "Morning"; // 06:00 → 13:59
					if (now >= new TimeSpan(14, 0, 0) && now < new TimeSpan(22, 0, 0))
						shift = "Evening";      // 14:00 → 21:59
					else if (now >= new TimeSpan(22, 0, 0) || now < new TimeSpan(6, 0, 0))
						shift = "Night";        // 22:00 → 05:59

					// ── تسجيل check-in لكل الـ Roles ────────────────────────
					using (var cmd = new OdbcCommand(@"
                        INSERT INTO Attendance
                            (employee_id, date, check_in_time, check_out_time,
                             shift_type, work_hours, overtime_hours)
                        VALUES
                            (?, GETDATE(), ?, NULL, ?, 0, 0)", con))
					{
						cmd.Parameters.AddWithValue("?", employeeId);
						cmd.Parameters.AddWithValue("?", DateTime.Now.TimeOfDay.ToString(@"hh\:mm\:ss"));
			...
			{
				// مش هنوقف الـ login بسبب خطأ في الحضور — بس هنعرض رسالة تحذير
				MessageBox.Show(
					"Login successful but attendance could not be recorded:\n" + ex.Message,
					"Attendance Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
			}
		}
		...
		private TabPage BuildRegisterTab(TabControl tabs)
		{
			...
			lblRegError.MaximumSize = new Size(292, 0);
			panel.Controls.Add(lblRegError);

			var btnRegister = MakePrimaryBtn("Create Account", Green, new Point(24, y + 24), 292);
			panel.Controls.Add(btnRegister);

			panel.AutoScrollMinSize = new Size(0, btnRegister.Bottom + 30);

			lblRegError.TextChanged += (s, e) =>
			{
				btnRegister.Top = string.IsNullOrEmpty(lblRegError.Text)
					? lblRegError.Top + 24
					: lblRegError.Bottom + 12;
				panel.AutoScrollMinSize = new Size(0, btnRegister.Bottom + 30);
			};
			...
			btnRegister.Click += (s, e) =>
			{
				...
				if (!string.IsNullOrEmpty(phone))
				{
					...
					{
						lblRegError.Text = "Phone number must contain only digits.";
						return;
					}
					if (phone.Length != 11)
					{
						lblRegError.Text = "Phone number must be exactly 11 digits.";
						return;
					}
				}
				...
				if (result == "SUCCESS")
				{
					...
					txtFullName.Clear(); txtEmail.Clear(); txtPhone.Clear();
					txtRegUser.Clear(); txtRegPass.Clear(); txtConfPass.Clear();
		...
		private void LoadDepartments(ComboBox cmb)
		{
			try
			{
				using (var con = new OdbcConnection(Global.ConnStr))
				using (var cmd = new OdbcCommand(
					"SELECT department_id, department_name FROM Department ORDER BY department_name", con))
				{
					con.Open();
		...
		private string RegisterUser(string username, string password,
			string fullName, string email, string phone, int departmentId)
		{
			try
			{
				using (var con = new OdbcConnection(Global.ConnStr))
				{
					...
					using (var cmd = new OdbcCommand(@"
                        INSERT INTO Employee
                            (employee_code, full_name, email, phone_number, department_id, hire_date)
                        VALUES (?, ?, ?, ?, ?, ?);
                        SELECT @@IDENTITY", con))
					{
						...
						using (var cmd2 = new OdbcCommand(@"
                            INSERT INTO [User]
                                (username, password_hash, employee_id, user_role, is_active, created_date)
                            VALUES (?, ?, ?, 'Employee', 1, ?)", con))
						{
							...
							cmd2.Parameters.AddWithValue("?", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
		...
		private Label MakeLabel(string text, Font font, Color color, Point loc)
			...
			{
				Text = text,
				Font = font ?? new Font("Segoe UI", 8.5f, FontStyle.Bold),
				ForeColor = color,
				AutoSize = true,
				Location = loc
			};
